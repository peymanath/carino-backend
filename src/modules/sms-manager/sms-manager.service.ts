...
export class SmsManagerService implements ProviderService {
  ...
  public async sendSmsWithPattern<TParams extends Array<object>>(mobile: string, patternId: number | string, parameters: TParams): Promise<ProviderServicePattern<TParams>> {
    if (registerEnv.SMS_PROVIDER === 'MELI') {
